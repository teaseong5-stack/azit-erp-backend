...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def booking_board_summary(request):
    today = timezone.now().date()
    ...
    weekly_schedules_qs = Reservation.objects.filter(
        start_date__gte=start_of_week, start_date__lte=end_of_week, status__in=['CONFIRMED', 'PAID', 'COMPLETED']
    ).select_related('customer', 'manager').order_by('start_date', 'tour_name')
    
    weekly_summary = weekly_schedules_qs.aggregate(
        total_sales=Coalesce(Sum('total_price'), Value(0, output_field=DecimalField())),
    ...
    # 3. Monthly Summary
    start_of_month = today.replace(day=1)
    monthly_schedules_qs = Reservation.objects.filter(
        start_date__year=today.year, start_date__month=today.month, status__in=['CONFIRMED', 'PAID', 'COMPLETED']
    ).select_related('customer', 'manager').order_by('start_date', 'tour_name')
    
    monthly_summary = monthly_schedules_qs.aggregate(
        total_sales=Coalesce(Sum('total_price'), Value(0, output_field=DecimalField())),
        total_count=Count('id')
    )
    
    return Response({
        'today_schedules': ReservationSerializer(today_schedules, many=True).data,
        'weekly_summary': weekly_summary,
        'weekly_schedules': ReservationSerializer(weekly_schedules_qs, many=True).data,
        'monthly_summary': monthly_summary,
        'monthly_schedules': ReservationSerializer(monthly_schedules_qs, many=True).data
    })